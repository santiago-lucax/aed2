...
class ColecaoRestaurantes {
    ...
    public static ColecaoRestaurantes lerCsv() {
        ...
        colecao.lerCsv("restaurantes.csv"); // chama o método da instância apontando para o arquivo no verde
...
public class Main {
    ...
    */
    public static void selecao(Restaurante[] arr, int n) {
        for (int i = 0; i < n - 1; i++) {
            int menor = i; // assume o atual é o menor elemento

            // laço para encontrar o menor elemento no restante do array
            for (int j = i + 1; j < n; j++) {
                comparacoes++; // incrementar contador de comparações entre chaves (nomes)
                int  cmp = arr[j].getNome().compareTo(arr[menor].getNome());
                ...
                // desempate por ID crescente
                    ...
                    comparacoes++;
                    if (arr[j].getId() < arr[menor].getId()) menor = j;
                }
            }
