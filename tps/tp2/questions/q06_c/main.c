...
/*
 * função principal para execucao do programa e leitura das entradas
*/

// variável global para registro de métricas apenas da pesquisa
int comparacoes = 0;

/*
 * algoritmo de ordenação por seleção (auxiliar)
 * necessário para ordenar o arranjo pelo nome antes da pesquisa binária
*/
 ...
 * busca a string informada no atributo nome do arranjo ordenado
*/
bool pesquisa_binaria(Restaurante *arr, int n, char *nome_buscado) {
    bool encontrou = false;
    int esq = 0;
    int dir = n - 1;

    // laço que divide o arranjo pela metade a cada iteração
    while (esq <= dir && !encontrou) {
        int meio = (esq + dir) / 2;
        comparacoes++; // incrementar contador de comparações da pesquisa
        
        int cmp = strcmp(nome_buscado, arr[meio].nome);

        if (cmp == 0) {
            encontrou = true; // flag para encerrar caso encontre
        } else if (cmp > 0) {
...
int main() {
    ...
    // --- PRIMEIRA PARTE: ler IDs da entrada padrao e filtrar do dataset ---
    while (continuar_leitura && fgets(linha_entrada, sizeof(linha_entrada), stdin) != NULL) {
        ...
        if (strlen(linha_entrada) > 0) {
            if (strcmp(linha_entrada, "FIM") == 0 || to_int(linha_entrada) == -1) {
                continuar_leitura = false; // flag para encerrar a primeira parte
            } else {
    ...
    // ordenar o arranjo previamente pelo nome para a pesquisa binária funcionar
    selecao(selecionados, n);

    // --- SEGUNDA PARTE: ler nomes da entrada padrao e realizar pesquisas ---
    continuar_leitura = true;
    double tempo_total = 0.0;

    while (continuar_leitura && fgets(linha_entrada, sizeof(linha_entrada), stdin) != NULL) {
        trim(linha_entrada);
        if (strlen(linha_entrada) > 0) {
            if (strcmp(linha_entrada, "FIM") == 0) {
                continuar_leitura = false; // encerra pesquisas
            } else {
                // cronometrar apenas o tempo exato da pesquisa
                clock_t inicio = clock();
                bool existe = pesquisa_binaria(selecionados, n, linha_entrada);
                clock_t fim = clock();
    ...
    // geração do arquivo de log com as métricas da pesquisa
    FILE *log = fopen("897498_binaria.txt", "w");
    if (log != NULL) {
        // formato pesquisa: matrícula [tab] comparações [tab] tempo total das pesquisas
        fprintf(log, "897498\t%d\t%.4f", comparacoes, tempo_total);
        fclose(log);
    }
