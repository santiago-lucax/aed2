...
public class Main {
    ...
    /*
     * algoritmo de ordenação mergesort (divisão)
     * ordena o arranjo dividindo recursivamente
    */
    public static void mergesort(Restaurante[] arr, int esq, int dir) {
        if (esq < dir) {
            int meio = (esq + dir) / 2;
    ...
    /*
     * algoritmo de ordenação mergesort (conquista)
     * intercala os sub-arranjos ordenando por cidade (desempate: nome)
    */
    public static void intercalar(Restaurante[] arr, int esq, int meio, int dir) {
        int n1 = meio - esq + 1;
        int n2 = dir - meio;
        ...
        // preencher arranjos temporários
        for (int i = 0; i < n1; i++) {
            a1[i] = arr[esq + i];
            movimentacoes++;
        }
        ...
        // intercalar verificando cidade e desempatando pelo nome
        while (i < n1 && j < n2) {
            comparacoes++;
            int cmpCidade = a1[i].getCidade().compareTo(a2[j].getCidade());
            
            // se a cidade for menor OU se for igual e o nome for menor
            if (cmpCidade < 0 || (cmpCidade == 0 && a1[i].getNome().compareTo(a2[j].getNome()) <= 0)) {
                arr[k] = a1[i];
                i++;
            } else {
                arr[k] = a2[j];
    ...
    public static void main(String[] args) {
        ...
        while (continuarLeitura && sc.hasNextLine()) {
            ...
            if (linhaEntrada.length() > 0) {
                ...
                if (idBuscado == -1) {
                    continuarLeitura = false; // flag para encerrar a leitura
                } else {
        ...
        // executar ordenação mergesort (índice 0 até n-1)
        if (n > 0) {
        ...
        try {
            PrintWriter log = new PrintWriter(new File("897498_mergesort.txt"));
