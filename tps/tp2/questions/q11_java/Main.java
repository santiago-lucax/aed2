...
/*
 * classe responsável por gerenciar a estrutura de dados Lista
 * utilizando alocação sequencial (arranjo)
*/
class Lista {
    private Restaurante[] array;
    private int n;

    // construtor da lista recebendo o tamanho máximo
    public Lista(int tamanho) {
        this.array = new Restaurante[tamanho];
        this.n = 0;
    }

    // insere um registro na primeira posição (0) e remaneja os demais
    public void inserirInicio(Restaurante r) throws Exception {
        if (this.n >= this.array.length) throw new Exception("Erro ao inserir: Lista cheia.");
        
        // levar os elementos uma posição para frente
    ...
    // insere um registro na posição informada e remaneja os demais
    public void inserir(Restaurante r, int pos) throws Exception {
        ...
        // levar os elementos uma posição para frente a partir de pos
    ...
    // remove o primeiro registro, remaneja os demais e retorna o item removido
    public Restaurante removerInicio() throws Exception {
        if (this.n == 0) throw new Exception("Erro ao remover: Lista vazia.");
        
        Restaurante resp = this.array[0];
        this.n--;
        
        // puxar os elementos uma posição para trás
    ...
    // remove o registro da posição informada, remaneja os demais e o retorna
    public Restaurante remover(int pos) throws Exception {
        ...
        Restaurante resp = this.array[pos];
        this.n--;
        
        // puxar os elementos uma posição para trás a partir de pos
    ...
    // remove o último registro e o retorna
    ...
    // exibe todos os elementos da lista chamando o método de formatação
    public void mostrar() {
        for (int i = 0; i < this.n; i++) {
            System.out.println(this.array[i].formatar());
        }
    }
}
...
public class Main {
     ...
     * no arranjo do dataset original de forma segura (sem break)
    */
    public static Restaurante buscarPorId(Restaurante[] csv, int tamanho, int idBuscado) {
        Restaurante resp = null;
        boolean encontrou = false;
        
        for (int i = 0; i < tamanho && !encontrou; i++) {
            if (csv[i] != null && csv[i].getId() == idBuscado) {
                resp = csv[i];
                encontrou = true;
            }
        }
    ...
    */
    public static void main(String[] args) {
        ...
        Lista lista = new Lista(200); // capacidade arbitrária que supra os testes do Verde
        Scanner sc = new Scanner(System.in);
        boolean continuarLeitura = true;

        // --- PRIMEIRA PARTE: popular a lista inserindo no fim ---
        while (continuarLeitura && sc.hasNextLine()) {
            ...
            if (linhaEntrada.length() > 0) {
                ...
                if (idBuscado == -1) {
                    continuarLeitura = false; // encerra primeira parte
                } else {
                    Restaurante r = buscarPorId(restaurantesCsv, tamanhoTotal, idBuscado);
                    if (r != null) {
                        try {
                            lista.inserirFim(r);
                        } catch (Exception e) { }
        ...
        if (sc.hasNextLine()) {
            int qtdComandos = Utils.toInt(sc.nextLine().trim());

            for (int k = 0; k < qtdComandos; k++) {
                String linhaCmd = sc.nextLine().trim();
                String[] partes = linhaCmd.split(" ");
                String cmd = partes[0];

                try {
                    if (Utils.isEquals(cmd, "II")) {
                        int id = Utils.toInt(partes[1]);
                        Restaurante r = buscarPorId(restaurantesCsv, tamanhoTotal, id);
                        if (r != null) lista.inserirInicio(r);
                    } 
                    else if (Utils.isEquals(cmd, "I*")) { // Comando com asterisco corrigido
                        int pos = Utils.toInt(partes[1]);
                        int id = Utils.toInt(partes[2]);
                        Restaurante r = buscarPorId(restaurantesCsv, tamanhoTotal, id);
                        if (r != null) lista.inserir(r, pos);
                    } 
                    else if (Utils.isEquals(cmd, "IF")) {
                        int id = Utils.toInt(partes[1]);
                        Restaurante r = buscarPorId(restaurantesCsv, tamanhoTotal, id);
                        if (r != null) lista.inserirFim(r);
                    } 
                    else if (Utils.isEquals(cmd, "RI")) {
                        Restaurante r = lista.removerInicio();
                        System.out.println("(R)" + r.getNome()); // Formatação sem espaço corrigida
                    } 
                    else if (Utils.isEquals(cmd, "R*")) { // Comando com asterisco corrigido
                        int pos = Utils.toInt(partes[1]);
                        Restaurante r = lista.remover(pos);
                        System.out.println("(R)" + r.getNome()); // Formatação sem espaço corrigida
                    } 
                    else if (Utils.isEquals(cmd, "RF")) {
                        Restaurante r = lista.removerFim();
                        System.out.println("(R)" + r.getNome()); // Formatação sem espaço corrigida
                    }
                } catch (Exception e) {
                    // ignora eventuais exceções conforme padrão do Verde
                }
            }
        }

        // exibir resultado final de como a lista ficou formatada
        lista.mostrar();
