...
/*
 * funções auxiliares de comparação para o quicksort
 * encapsulam a lógica de comparação dupla (avaliação e desempate por nome)
*/
bool is_menor(Restaurante r1, Restaurante r2) {
    comparacoes++;
    if (r1.avaliacao < r2.avaliacao) return true;
    
    if (r1.avaliacao == r2.avaliacao) {
        comparacoes++;
        if (strcmp(r1.nome, r2.nome) < 0) return true;
    }
    return false;
}

bool is_maior(Restaurante r1, Restaurante r2) {
    comparacoes++;
    if (r1.avaliacao > r2.avaliacao) return true;
 ...
 * ordena o arranjo de restaurantes pelo atributo avaliacao (desempate: nome)
*/
void quicksort(Restaurante *arr, int esq, int dir) {
    ...
    // laço para particionar o arranjo
    while (i <= j) {
        while (is_menor(arr[i], pivo)) {
            i++;
        }
        while (is_maior(arr[j], pivo)) {
            j--;
        }
        ...
        if (i <= j) {
            ...
            arr[i] = arr[j];
            arr[j] = temp;
            movimentacoes += 3; // cada swap contabiliza 3 movimentações
            i++;
            j--;
        }
    }
...
int main() {
    ...
    FILE *log = fopen("897498_quicksort.txt", "w");
    if (log != NULL) {
