...
*/
public class Main {
    // variável global para registro de métricas de pesquisa (não há movimentações)
    public static int comparacoes = 0;

    /*
     * algoritmo de pesquisa sequencial
     * busca a string informada no atributo nome do arranjo de restaurantes
    */
    public static boolean pesquisaSequencial(Restaurante[] arr, int n, String nomeBuscado) {
        boolean encontrou = false;
        
        // laço para percorrer o arranjo verificando a chave
        for (int i = 0; i < n && !encontrou; i++) {
            comparacoes++; // incrementar contador de comparações
            if (Utils.isEquals(arr[i].getNome(), nomeBuscado)) {
                encontrou = true; // flag encerra a busca antecipadamente caso encontre
            }
        }
        
        return encontrou; // retorna se a chave existe ou não no dataset
    }
    ...
    */
    public static void main(String[] args) {
        ...
        Restaurante[] selecionados = new Restaurante[1000]; // arranjo que servirá de base
        int n = 0;
        Scanner sc = new Scanner(System.in);
        boolean continuarLeitura = true; // flag para controle de laço
        ...
        while (continuarLeitura && sc.hasNextLine()) {
            ...
            if (linhaEntrada.length() > 0) {
                if (Utils.isEquals(linhaEntrada, "FIM") || Utils.toInt(linhaEntrada) == -1) {
                    continuarLeitura = false; // encerra a primeira parte da leitura
                } else {
                    int idBuscado = Utils.toInt(linhaEntrada);
                    boolean encontrou = false;

                    // preencher a base de dados em memória para pesquisa
        ...
        // --- SEGUNDA PARTE DA ENTRADA: Pesquisas pelo atributo nome ---
        continuarLeitura = true; // reativa a flag
        long tempoTotal = 0;

        // laço processa as pesquisas até o fim das entradas ou "FIM"
        while (continuarLeitura && sc.hasNextLine()) {
            String nomeBuscado = sc.nextLine().trim();

            if (nomeBuscado.length() > 0) {
                if (Utils.isEquals(nomeBuscado, "FIM")) {
                    continuarLeitura = false; // flag para encerrar pesquisas
                } else {
                    // cronometrar apenas o tempo da pesquisa
                    long inicio = System.currentTimeMillis();
                    boolean existe = pesquisaSequencial(selecionados, n, nomeBuscado);
                    long fim = System.currentTimeMillis();
                    tempoTotal += (fim - inicio); // acumular tempo de todas as pesquisas

                    // exibir resultado da busca na saída padrão
                    if (existe) {
        ...
        try {
            PrintWriter log = new PrintWriter(new File("897498_sequencial.txt"));
            // formato pesquisa: matrícula [tab] comparações [tab] tempo de todas as pesquisas
            log.printf("897498\t%d\t%d\n", comparacoes, tempoTotal);
            log.close();
        } catch (Exception e) {
