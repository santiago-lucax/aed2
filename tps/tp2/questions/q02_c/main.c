...
int main() {
    ...
    ler_csv(&colecao, "/tmp/restaurantes.csv"); // caminho no padrao verde

    char linha_entrada[100];
    bool continuar_leitura = true; // flag para substituir o break do laco principal

    // laco de repeticao para ler as entradas controlando por flag
    while (continuar_leitura && fgets(linha_entrada, sizeof(linha_entrada), stdin) != NULL) {
        trim(linha_entrada);

        // processar apenas linhas que nao estao em branco (substitui o uso de continue)
        if (strlen(linha_entrada) > 0) {
            // utilizar função propria para converter string em um inteiro
            int id_buscado = to_int(linha_entrada);

            // condicao de parada (fim da leitura solicitado pelo pub.in)
            if (id_buscado == -1) {
                continuar_leitura = false; // encerra o laco externo na proxima verificacao
            } else {
                bool encontrou = false; // flag para substituir o break da busca
                
                // laco para busca linear pelo identificador controlando por flag para nao usar break
                for (int i = 0; i < colecao.tamanho && !encontrou; i++) {
                    // verificacao para validar o id
                    if (colecao.restaurantes[i].id == id_buscado) {
                        char buffer_saida[2000]; // buffer da saida aumentado para garantir que caiba toda a string final
                        formatar_restaurante(&(colecao.restaurantes[i]), buffer_saida); 
                        printf("%s\n", buffer_saida); // exibir dados formatados no padrao esperado
                        encontrou = true; // encerra a iteracao do for gracas a condicao !encontrou
                    }
                }
            }
        }
    ...
    // liberacao da memoria dinamica alocada pela biblioteca padrao
