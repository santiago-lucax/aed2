...
public class Main {
    public static void main(String[] args) {
        ...
        // preparar o scanner para ler a entrada padrão
        Scanner sc = new Scanner(System.in);

        // laço de repetição para ler as entradas até o final do arquivo de teste
        while (sc.hasNextLine()) {
            String linha = sc.nextLine().trim();

            // ignorar linhas em branco da entrada
            if (linha.isEmpty()) {
                continue;
            }

            // utilizar função estática para converter string em um inteiro
            int idBuscado = Utils.toInt(linha);

            // condição de parada (fim da leitura solicitado pelo pub.in)
            if (idBuscado == -1) {
                break;
            }

            // laço para busca linear pelo identificador no arranjo
            for (int i = 0; i < tamanhoColecao; i++) {
                // verificação de segurança extra para evitar nulos e validar o id
                if (restaurantes[i] != null && restaurantes[i].getId() == idBuscado) {
                    System.out.println(restaurantes[i].formatar()); // exibir dados formatados no padrão esperado
                    break; // interromper a busca após encontrar o restaurante correto
                }
            }
        }

        sc.close(); // fechar scanner para liberar o recurso
    }
} // fim Main
