...
// variáveis globais para registro de métricas
int comparacoes = 0;
int movimentacoes = 0;
...
int main() {
    ...
    Restaurante selecionados[1000]; // arranjo para armazenar apenas os pesquisados
    int n = 0;
    ...
    while (continuar_leitura && fgets(linha_entrada, sizeof(linha_entrada), stdin) != NULL) {
        ...
        if (strlen(linha_entrada) > 0) {
            ...
            if (id_buscado == -1) {
                continuar_leitura = false; // flag para encerrar a leitura conforme pub.in
            } else {
