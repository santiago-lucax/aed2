...
/*
 * classe responsável por gerenciar a estrutura Fila Circular
 * com limite de 5 itens.
*/
class FilaCircular {
    ...
    // a fila terá tamanho 5, usamos o array com 6 posições por segurança modular
    public FilaCircular(int tamanho) {
        this.array = new Restaurante[tamanho + 1];
        this.primeiro = 0;
        this.ultimo = 0;
    }
    ...
    // identifica se a fila está cheia
    public boolean isCheia() {
        return ((this.ultimo + 1) % this.array.length) == this.primeiro;
    }

    // método para calcular e retornar a média de anos da fila atual
    public int getMediaAno() {
        ...
        while (i != this.ultimo) {
            soma += this.array[i].getDataAbertura().getAno();
            count++;
            i = (i + 1) % this.array.length;
        }
        
        if (count == 0) return 0;
        // Float garante arredondamento correto pela matemática (ex: 2012.5 -> 2013)
        return Math.round((float) soma / count);
    }

    // remove o primeiro item da fila circular e o retorna
    public Restaurante remover() throws Exception {
        if (isVazia()) throw new Exception("Fila vazia");
    ...
    // enfileira verificando e removendo antes se estiver cheia
    public void enfileirar(Restaurante r) throws Exception {
        if (isCheia()) {
            Restaurante removido = remover();
            System.out.println("(R)" + removido.getNome());
        }
        
        this.array[this.ultimo] = r;
        this.ultimo = (this.ultimo + 1) % this.array.length;
        
        // conforme especificação, toda vez que insere na fila, imprime a nova média
        System.out.println("(I)" + getMediaAno());
    }
...
public class Main {
    ...
    */
    public static void main(String[] args) {
        ...
        FilaCircular fila = new FilaCircular(5); // Inicializa com o tamanho de 5 elementos exigido
        Scanner sc = new Scanner(System.in);
        boolean continuarLeitura = true;

        // --- PRIMEIRA PARTE: popular a fila inserindo ---
        ...
        // --- SEGUNDA PARTE: processamento dos comandos ---
        if (sc.hasNextLine()) {
